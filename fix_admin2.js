...
export default function AdminPage() {
  ...
  const [orgIdProyecto, setOrgIdProyecto] = useState('');
  ...
  useEffect(() => {
    if (tab !== 'proyectos' || !orgIdProyecto) {
      setProyectos([]);
      return;
    }
    const cargarProyectos = async () => {
      const snap = await getDocs(collection(db, 'organizaciones', orgIdProyecto, 'proyectos'));
  ...
  const guardarOrganizacion = async (e) => {
    e.preventDefault();
    if (!nombreOrg.trim()) { setMensaje('Falta el nombre de la organización.'); return; }
    setGuardando(true);
    if (editandoOrgId) {
      await updateDoc(doc(db, 'organizaciones', editandoOrgId), { nombre: nombreOrg.trim() });
      setMensaje('Organización actualizada.');
    } else {
      await addDoc(collection(db, 'organizaciones'), { nombre: nombreOrg.trim(), fechaCreacion: serverTimestamp() });
  ...
  const guardarProyecto = async (e) => {
    e.preventDefault();
    if (!nombreProyecto.trim() || !orgIdProyecto) { setMensaje('Falta nombre o seleccionar organización.'); return; }
    setGuardando(true);
    const datos = {
      nombre: nombreProyecto.trim(),
      descripcion: descripcionProyecto.trim(),
      organizacionId: orgIdProyecto,
      fechaCreacion: serverTimestamp(),
    };
    if (editandoProyectoId) {
      await updateDoc(doc(db, 'organizaciones', orgIdProyecto, 'proyectos', editandoProyectoId), datos);
      setMensaje('Proyecto actualizado.');
    } else {
      await addDoc(collection(db, 'organizaciones', orgIdProyecto, 'proyectos'), datos);
  ...
  const guardarEquipo = async (e) => {
    e.preventDefault();
    if (!nombreEquipo.trim() || !orgIdEquipo) { setMensaje('Falta el nombre del equipo o la organización.'); return; }
    setGuardando(true);
    const datos = { nombre: nombreEquipo.trim(), orgId: orgIdEquipo, supervisorId: supervisorIdEquipo.trim() || null };
    if (editandoEquipoId) {
  ...
  const quitarFilaTarea = (index) => { setTareas(tareas.filter((_, i) => i !== index)); };
  ...
  const limpiarFormularioCargo = () => {
    setNombreCargo(''); setNivelCargo('operativo'); setOrgIdCargo('');
  ...
  const iniciarEdicionCargo = (cargo) => {
    ...
    setTareas(cargo.tareasCriticas && cargo.tareasCriticas.length > 0
      ...
      : [{ id: generarId(), nombre: '', descripcion: '', criticidad: 'media', nivel: 'operativo' }]);
    setEditandoCargoId(cargo.id); setMensaje('');
  };

  const guardarCargo = async (e) => {
    e.preventDefault();
    if (!nombreCargo.trim() || !orgIdCargo) { setMensaje('Falta el nombre del cargo o la organización.'); return; }
    setGuardando(true);
    const datos = {
      nombre: nombreCargo.trim(), nivel: nivelCargo, orgId: orgIdCargo,
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={() => router.push('/dashboard')} className="text-sm bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded-md transition">
          Volver al dashboard
        </button>
      </header>

      <div className="flex border-b border-gray-800 px-6">
        {['organizaciones', 'proyectos', 'equipos', 'cargos'].map((t) => (
          <button
            key={t}
        ...
        {tab === 'organizaciones' && (
          <div>
            <form onSubmit={guardarOrganizacion} className="flex gap-2 mb-6">
              ...
              {editandoOrgId && (
                <button type="button" onClick={limpiarFormularioOrg} className="bg-gray-800 hover:bg-gray-700 px-4 py-2 rounded-md text-sm font-medium">
                  Cancelar
                </button>
              )}
            </form>
              ...
              <>
                ...
                <div className="space-y-2">
                  {proyectos.length === 0 ? (
                    ...
                    proyectos.map((p) => (
                      <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-md px-4 py-3 flex items-center justify-between">
                        <div>
                          <p className="font-medium text-sm">{p.nombre}</p>
                          {p.descripcion && <p className="text-xs text-gray-500">{p.descripcion}</p>}
                          <p className="text-xs text-gray-600 mt-1">ID: {p.id}</p>
                        </div>
                        <button onClick={() => iniciarEdicionProyecto(p)} className="text-sm text-blue-400 hover:text-blue-300">Editar</button>
                      </div>
                    ))
                  )}
                </div>
          ...
          <div>
            <form onSubmit={guardarEquipo} className="space-y-3 mb-6">
              <input value={nombreEquipo} onChange={(e) => setNombreEquipo(e.target.value)} placeholder="Nombre del equipo"
                className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500" />
              <select value={orgIdEquipo} onChange={(e) => setOrgIdEquipo(e.target.value)}
                className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500">
                <option value="">Selecciona organización</option>
                {organizaciones.map((org) => (<option key={org.id} value={org.id}>{org.nombre}</option>))}
              </select>
              <input value={supervisorIdEquipo} onChange={(e) => setSupervisorIdEquipo(e.target.value)}
              ...
              <div className="flex gap-2">
                ...
                {editandoEquipoId && (
                  <button type="button" onClick={limpiarFormularioEquipo} className="bg-gray-800 hover:bg-gray-700 px-4 py-2 rounded-md text-sm font-medium">Cancelar</button>
                )}
              </div>
            </form>
            <div className="space-y-2">
              {equipos.map((eq) => (
                <div key={eq.id} className="bg-gray-900 border border-gray-800 rounded-md px-4 py-3 flex items-center justify-between">
                  <div>{eq.nombre}<span className="text-xs text-gray-500 ml-2">org: {eq.orgId}</span></div>
                  <button onClick={() => iniciarEdicionEquipo(eq)} className="text-sm text-blue-400 hover:text-blue-300">Editar</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'cargos' && (
          <div>
            <form onSubmit={guardarCargo} className="space-y-3 mb-6">
              <input value={nombreCargo} onChange={(e) => setNombreCargo(e.target.value)} placeholder="Nombre del cargo"
                className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500" />
              <select value={nivelCargo} onChange={(e) => setNivelCargo(e.target.value)}
                className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500">
                {NIVELES.map((n) => (<option key={n} value={n}>{n}</option>))}
              </select>
              <select value={orgIdCargo} onChange={(e) => setOrgIdCargo(e.target.value)}
                className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500">
                <option value="">Selecciona organización</option>
                {organizaciones.map((org) => (<option key={org.id} value={org.id}>{org.nombre}</option>))}
              </select>
              <div>
                ...
                {tareas.map((t, i) => (
                  <div key={t.id} className="grid grid-cols-12 gap-2 mb-2">
                    <input value={t.nombre} onChange={(e) => actualizarTarea(i, 'nombre', e.target.value)} placeholder="Tarea"
                      className="col-span-4 bg-gray-900 border border-gray-800 rounded-md px-2 py-1.5 text-sm" />
                    <input value={t.descripcion} onChange={(e) => actualizarTarea(i, 'descripcion', e.target.value)} placeholder="Descripción"
                      className="col-span-4 bg-gray-900 border border-gray-800 rounded-md px-2 py-1.5 text-sm" />
                    <select value={t.criticidad} onChange={(e) => actualizarTarea(i, 'criticidad', e.target.value)}
                      className="col-span-2 bg-gray-900 border border-gray-800 rounded-md px-2 py-1.5 text-sm">
                      <option value="alta">alta</option>
                      <option value="media">media</option>
                      <option value="baja">baja</option>
                    </select>
                    <select value={t.nivel} onChange={(e) => actualizarTarea(i, 'nivel', e.target.value)}
                      className="col-span-1 bg-gray-900 border border-gray-800 rounded-md px-2 py-1.5 text-sm">
                      {NIVELES.map((n) => (<option key={n} value={n}>{n.slice(0, 3)}</option>))}
                    </select>
                    <button type="button" onClick={() => quitarFilaTarea(i)} className="col-span-1 text-amber-500 hover:text-amber-400 text-sm">✕</button>
                  </div>
                ))}
                <button type="button" onClick={agregarFilaTarea} className="text-sm text-blue-400 hover:text-blue-300">+ Agregar tarea</button>
              </div>
              <div className="flex gap-2">
                <button disabled={guardando} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 px-4 py-2 rounded-md text-sm font-medium">
                ...
                {editandoCargoId && (
                  <button type="button" onClick={limpiarFormularioCargo} className="bg-gray-800 hover:bg-gray-700 px-4 py-2 rounded-md text-sm font-medium">Cancelar</button>
                )}
              </div>
            </form>
            <div className="space-y-2">
              {cargos.map((c) => (
                <div key={c.id} className="bg-gray-900 border border-gray-800 rounded-md px-4 py-3 flex items-center justify-between">
                  ...
                  <button onClick={() => iniciarEdicionCargo(c)} className="text-sm text-blue-400 hover:text-blue-300">Editar</button>
                </div>
              ))}
            </div>
          </div>
