...
export default function InstructivosPage() {
  ...
  const [proyectosOrg, setProyectosOrg] = useState([]);
  ...
  const seleccionarCargo = async (cargo) => {
    ...
    console.log('orgId del cargo:', cargo.orgId);

    if (cargo.orgId) {
      try {
        const proySnap = await getDocs(collection(db, 'organizaciones', cargo.orgId, 'proyectos'));
        console.log('Proyectos encontrados:', proySnap.docs.length);
        setProyectosOrg(proySnap.docs.map((d) => ({ id: d.id, ...d.data() })));
      } catch (err) {
      ...
      tareas.map(async (t) => {
        ...
        if (snap.exists()) {
          resultado[t.id] = {
            ...
            variables: snap.data().variables || {},
          };
        } else {
          resultado[t.id] = { texto: '', audioBase64: null, audioDesactualizado: false, textoGuardado: '', variables: {} };
        }
      })
    );
  ...
  const actualizarTexto = (tareaId, texto) => {
    setInstructivos((prev) => ({ ...prev, [tareaId]: { ...prev[tareaId], texto } }));
  };
  ...
  const actualizarVariable = (tareaId, varName, proyectoId, valor) => {
    setInstructivos((prev) => ({
      ...prev,
      [tareaId]: {
        ...prev[tareaId],
        variables: {
          ...prev[tareaId].variables,
          [varName]: { ...prev[tareaId].variables?.[varName], [proyectoId]: valor },
        },
      },
    }));
  };
  ...
  const guardarInstructivo = async (tareaId) => {
    ...
    const variables = instructivos[tareaId]?.variables || {};

    await setDoc(ref_doc, {
      cargoId: cargoSeleccionado.id, tareaId, texto, variables,
      autorId: usuario.uid, autorRol: usuario.rol, fecha: serverTimestamp(),
    ...
    } catch (err) { console.error('Error generando flujo:', err); }
    ...
    try {
      ...
      if (resAudio.ok) {
        ...
        await setDoc(ref_doc, { audioBase64: base64, audioGeneradoEn: serverTimestamp(), audioDesactualizado: false }, { merge: true });
        setInstructivos((prev) => ({
          ...prev,
          [tareaId]: { ...prev[tareaId], audioBase64: base64, audioDesactualizado: false, textoGuardado: texto },
        }));
  ...
  if (cargando || cargandoDatos) {
    return <div className="min-h-screen bg-gray-950 flex items-center justify-center"><p className="text-gray-500">Cargando...</p></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="border-b border-gray-800 px-6 py-4">
        ...
        <p className="text-xs text-gray-500">Usa {'{{variable}}'} para valores que cambian por proyecto.</p>
      </header>
        ...
        {cargoSeleccionado && (
          <div className="space-y-6">
            {(cargoSeleccionado.tareasCriticas || []).map((t) => {
              ...
              const varsDetectadas = detectarVariables(inst.texto || '');
              return (
                <div key={t.id} className="border-b border-gray-800 pb-6">
                  ...
                  <textarea value={inst.texto || ''} onChange={(e) => actualizarTexto(t.id, e.target.value)}
                    placeholder="¿Cómo se hace bien esta tarea? Usa {{variable}} para valores que cambian por proyecto."
                    rows={3} className="w-full bg-gray-900 border border-gray-800 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-blue-500" />

                  {varsDetectadas.length > 0 && proyectosOrg.length > 0 && (
                    <div className="mt-3 bg-gray-900 border border-gray-700 rounded-md p-3">
                      <p className="text-xs text-teal-400 mb-3">Variables detectadas — define el valor por proyecto:</p>
                      <div className="space-y-3">
                        {varsDetectadas.map((varName) => (
                          <div key={varName}>
                            <p className="text-xs font-mono text-amber-400 mb-1">{'{{' + varName + '}}'}</p>
                            <div className="space-y-1 pl-2">
                              {proyectosOrg.map((proyecto) => (
                                <div key={proyecto.id} className="flex items-center gap-2">
                                  <span className="text-xs text-gray-500 w-28 truncate">{proyecto.nombre}</span>
                                  <input
                                    value={inst.variables?.[varName]?.[proyecto.id] || ''}
                                    onChange={(e) => actualizarVariable(t.id, varName, proyecto.id, e.target.value)}
                                    placeholder="valor para este proyecto"
                                    className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs focus:outline-none focus:border-teal-500"
                                  />
                                </div>
                              ))}
                            </div>
                  ...
                  {varsDetectadas.length > 0 && proyectosOrg.length === 0 && (
                    <p className="text-xs text-amber-400 mt-2">⚠ Variables detectadas pero no hay proyectos configurados para este cargo. Créalos en Admin → Proyectos.</p>
                  )}
                  <div className="mt-2 flex items-center gap-3">
                    <GrabadorAudio onTranscripcion={(texto) => agregarTranscripcion(t.id, texto)} />
                    <button onClick={() => guardarInstructivo(t.id)} disabled={guardando[t.id]}
